/**
 * Displays bookings for a specific venue, supporting both mobile (card view with navigation)
 * and desktop (table view) layouts. Fetches venue data from a global store and allows
 * navigation between bookings on mobile. Shows booking details such as customer, guests,
 * check-in/out dates, duration, total price, and creation date.
 *
 * @component
 * @param {Object} props
 * @param {string|number} props.venueId - The ID of the venue to display bookings for.
 * @param {Function} props.onClose - Callback function to close the bookings view.
 * @returns {JSX.Element|null} The rendered bookings component or null if not visible.
 */
      ...
      <div className={vmProfileStyles.mobileViewBookings}>
        <div style={{ textAlign: "center", padding: "20px" }}>
          ...
          {venueBookings.length > 0 ? (
            ...
            >
              <h3>
                Booking {currentBookingIndex + 1} of {venueBookings.length}
              </h3>
              <p>
                <strong>Customer:</strong>{" "}
                {venueBookings[currentBookingIndex].customer?.name || "Unknown"}
              </p>
              <p>
              ...
              <p>
                ...
                {venue?.price *
                  Math.max(
                    1,
                    Math.round(
                      (new Date(venueBookings[currentBookingIndex].dateTo) -
                        new Date(venueBookings[currentBookingIndex].dateFrom)) /
                        (1000 * 60 * 60 * 24)
                    )
                  )}{" "}
                NOK
          ...
          <div
            className={vmProfileStyles.buttonDivPopup}
                ...
                venueBookings.map((booking, index) => {
                  ...
                  return (
                    <TableRow key={`${booking.id || `fallback`}-${index}`}>
                      <TableCell className={gStyles.bodyWhite}>
